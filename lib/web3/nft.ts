import { NFTMetadataOrUri } from '@thirdweb-dev/sdk';

export class NFT {
  metadata: Record<string, any> = {};

  async insert(data: Record<string, any>) {
    Object.keys(data).forEach((key) => {
      if (data[key] instanceof Object) {
        this.metadata[key] = { ...this.metadata[key], [key]: { ...data[key] } };
      } else if (data[key] instanceof Array) {
        this.metadata[key] = [...this.metadata[key], data[key]];
      } else {
        this.metadata[key] = data[key];
      }
    });
  }

  public toObject() {
    return this.metadata;
  }
}

// import {
//   Canvas,
//   CanvasRenderingContext2D,
//   createCanvas,
//   loadImage
// } from 'canvas';

// export class ImageNFT extends NFT {
//   canvas: Canvas;
//   ctx: CanvasRenderingContext2D;

//   constructor() {
//     super();
//     this.canvas = createCanvas(550, 550);
//     this.ctx = this.canvas.getContext('2d');
//     this.ctx.globalAlpha = 1;
//     this.ctx.globalCompositeOperation = 'source-over';
//   }

//   public getMeta() {
//     this.meta.image = this.canvas.toDataURL('image/png');
//     return this.meta;
//   }

//   override async insertMeta(data: { [key: string]: any }) {
//     const image = await loadImage(data.image);
//     this.ctx.drawImage(image, 0, 0, 500, 500);
//     delete data.image;

//     Object.keys(data).forEach((key) => {
//       if (key in NFTInputOrUriSchema) {
//         if (data[key] instanceof Object) {
//           this.meta[key] = { ...this.meta[key], [key]: { ...data[key] } };
//         } else if (data[key] instanceof Array) {
//           this.meta[key] = [...this.meta[key], data[key]];
//         } else {
//           this.meta[key] = data[key];
//         }
//       }
//     });
//   }
// }
